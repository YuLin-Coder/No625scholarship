  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr>
        ...
        <th align="center" valign="middle" class="borderright">评审方式</th>
  
         <th align="center" valign="middle" class="borderright">投票数</th>
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <td align="center" valign="middle" class="borderright borderbottom">${data[8] }</td>
        <c:if test="${data[8] eq '需投票'}">
        <td align="center" valign="middle" class="borderright borderbottom">${data[9] }</td>
        </c:if>
                <c:if test="${data[8] eq '不投票'}">
        <td align="center" valign="middle" class="borderright borderbottom">不计票</td>
        </c:if>
       <c:if test="${role!='学生' }">
        <td align="center" valign="middle" class="borderbottom">
           <a href="<%=path %>/applyServlet?operate=modify&id=${data[0]}" target="mainFrame" onFocus="this.blur()" class="add">编辑</a><span class="gray">&nbsp;|&nbsp;</span>
           <a href="<%=path %>/applyServlet?operate=del&id=${data[0]}" target="mainFrame" onFocus="this.blur()" class="add">删除</a>
        </td>
        </c:if>
        <c:if test="${(role eq '学生') and (data[8] eq '需投票') }">
        <td align="center" valign="middle" class="borderbottom">
           <a href="<%=path %>/applyServlet?operate=select&id=${data[0]}" target="mainFrame" onFocus="this.blur()" class="add">投票</a>
        </td>
        </c:if>
